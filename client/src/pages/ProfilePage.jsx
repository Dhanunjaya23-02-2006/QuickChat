      ...
      rounded-lg">
        <form onSubmit={handleSubmit} className="flex flex-col gap-5 p-10 flex-1">
          ...
          <label htmlFor="avatar" className="flex items-center gap-3 cursor-pointer">
            ...
            <img src={selectedImg ? URL.createObjectURL(selectedImg) : assets.avatar_icon } alt="" className="w-12 h-12 rounded-full object-cover"/>
              upload profile image
          </label>
          <input onChange={(e)=>
            setName(e.target.value) 
          } value={name} type="text" required placeholder="your name.." className="p-2 border border-gray-500
        ...
        <div className="flex flex-1 items-center justify-center p-10 max-sm:p-6">
          <img className="w-44 h-44 rounded-full object-cover" src={authUser?.profilePic || assets.logo_icon} alt="" />
        </div>
      </div>
    </div>
  )
